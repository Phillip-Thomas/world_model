"""
Temporal Visual World Model (v1.2)
===================================
Extends v1.1 with deeper network and flexible token grid.

Key changes from v1.1:
- 10 layers instead of 8 (deeper network for better world modeling)
- Flexible token grid size (supports 21x16 for 84x64 aspect-preserved input)
...
class TemporalVisualWorldModel(nn.Module):
    ...
    def forward(
        ...
        Returns:
            logits: (B, N, vocab) predictions
            loss: CE loss if target provided
        """
    ...
    def predict_next_frame(
        ...
        # Forward
        logits, _ = self.forward(frame_history, action_tensor)
...
if __name__ == "__main__":
    print("=" * 60)
    print("Temporal Visual World Model (v1.2) Test")
    print("=" * 60)
    ...
    print(f"Layers: 10 (was 8 in v1.1)")
    ...
    logits, loss = model(frame_history, actions, target)
    ...
    print("\n--- Test 2: Legacy Square (64x64 -> 16x16 tokens) ---")
    ...
    logits_legacy, loss_legacy = model_legacy(frame_history_legacy, actions, target_legacy)
    print(f"  Input history: {frame_history_legacy.shape}")
    print(f"  Output logits: {logits_legacy.shape}")
    
    # Test inference with buffer
    print("\n--- Test 3: Inference with Frame Buffer ---")
    ...
    print("\n[OK] Temporal world model v1.2 working!")
